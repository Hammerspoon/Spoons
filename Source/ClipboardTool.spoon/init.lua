...
--- ClipboardTool.honor_ignoredWindows
--- Variable
--- If `true`, check whether the current focused window is among `obj.ignoredWindows`, and ignore
--- the supplied data. Defaults to `true`
obj.honor_ignoredWindows = true

--- ClipboardTool.ignoredWindows
--- Variable
--- Window name prefixes to ignore.
---
--- When 1Password persists items in clipboard, its bundle ID (see `obj.ignoredIdentifiers`) is
--- available among the contents (see `shouldBeStored()`). Therefore, the approach with detecting
--- the application using its bundle ID is viable. Note that when you use the copy keyboard
--- shortcut, some copied items will still appear in the clipboard history; use the UI buttons!
---
--- Bitwarden, and probably most other applications, doesn't work this way. When copying anything
...
function obj:shouldBeStored()
   ...
   if self:isIgnoredWindowActive() then
      return false
   end
...
function obj:isIgnoredWindowActive()
   ...
   local focusedWindowTitle = hs.window.focusedWindow():title()
   return self.ignoredWindows[focusedWindowTitle] or false
end
